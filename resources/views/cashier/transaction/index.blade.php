    ...
    <div class="flex flex-1 overflow-hidden h-full">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-0 bg-background-light relative h-full">
            ...
            <div class="flex-1 overflow-y-auto p-6 pt-2">
                ...
                <!-- Load More Button -->
                <div x-show="hasMore && !search && category === 'all'" class="mt-6 flex justify-center">
                    <button @click="loadMore()" 
                        :disabled="loadingMore"
                        class="inline-flex items-center gap-2 px-6 py-3 bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-xl hover:border-blue-500 hover:text-blue-600 transition-all disabled:opacity-50">
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('cashierApp', () => ({
                ...
                async resetToInitial() {
                    ...
                    try {
                        const response = await fetch(`{{ route('cashier.transaction.index') }}?page=1`, {
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest'
                            }
                        });
                ...
                async loadMore() {
                    ...
                    try {
                        const response = await fetch(`{{ route('cashier.transaction.index') }}?page=${this.currentPage + 1}`, {
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest'
                            }
                        });
                ...
                get filteredMedicines() {
                    ...
                    const sourceArray = this.searchResults !== null ? this.searchResults : this.medicines;
                    
                    return sourceArray.filter(medicine => {

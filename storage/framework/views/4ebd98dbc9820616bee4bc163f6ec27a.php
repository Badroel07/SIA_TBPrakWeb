...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Kasir - ePharma'); ?></title>
...
<body class="bg-slate-100 font-sans text-slate-900 h-screen overflow-hidden flex" <?php echo $__env->yieldContent('bodyAttributes'); ?>>
    ...
    <aside class="w-72 bg-white m-4 rounded-3xl shadow-xl flex flex-col shrink-0 relative overflow-hidden">
        ...
        <nav class="flex-1 px-4 space-y-3 overflow-y-auto py-4">
            ...
            <a href="<?php echo e(route('cashier.dashboard')); ?>" 
               class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group font-display <?php echo e($activeMenu === 'dashboard' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 translate-x-1' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600 hover:translate-x-1'); ?>">
            ...
            <a href="<?php echo e(route('cashier.transaction.index')); ?>" 
               class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group font-display <?php echo e($activeMenu === 'pos' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 translate-x-1' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600 hover:translate-x-1'); ?>">
            ...
            <a href="<?php echo e(route('cashier.orders.incoming')); ?>" 
               class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group font-display <?php echo e($activeMenu === 'orders' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 translate-x-1' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600 hover:translate-x-1'); ?>">
                <div class="relative">
            ...
            <a href="<?php echo e(route('cashier.transaction.history')); ?>" 
               class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group font-display <?php echo e($activeMenu === 'history' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 translate-x-1' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600 hover:translate-x-1'); ?>">
        ...
        <div class="p-4 mt-auto">
            <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-bold text-slate-900 truncate font-display"><?php echo e(Auth::user()->name); ?></p>
        ...
        <?php if(session('success')): ?>
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                ...
                <div>
                    ...
                    <p class="text-xs text-green-100"><?php echo e(session('success')); ?></p>
                </div>
        ...
        <?php if(session('error')): ?>
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                ...
                <div>
                    ...
                    <p class="text-xs text-red-100"><?php echo e(session('error')); ?></p>
                </div>
                <button @click="show = false" class="ml-2 opacity-70 hover:opacity-100"><span class="material-symbols-outlined text-sm">close</span></button>
            </div>
        <?php endif; ?>
    ...
    <div id="dynamicToast" 
        ...
        <div class="flex gap-2 ml-4">
            <a href="<?php echo e(route('cashier.orders.incoming')); ?>" class="px-4 py-2 bg-white font-bold rounded-xl transition-colors flex items-center gap-1 shadow-lg shadow-black/5" style="pointer-events: auto;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function pollOrderCounts() {
                ...
                try {
                    const res = await fetch('<?php echo e(route("cashier.orders.counts")); ?>');
...
<?php /**PATH C:\laragon\www\SIA_TBPrakWeb\resources\views/cashier/layouts/app.blade.php ENDPATH**/ ?>
...
<div x-data="{ 
    ...
    isCustomer: <?php echo e(Auth::check() && Auth::user()->isCustomer() ? 'true' : 'false'); ?>,
    loadingCart: false,
    ...
    fetchServerCart() {
        ...
        fetch('<?php echo e(route('customer.cart.index')); ?>', {
    ...
    updateQty(id, change) {
        if (this.isCustomer) {
            ...
            fetch('<?php echo e(route('customer.cart.update')); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    removeItem(id) {
        if (this.isCustomer) {
            fetch('<?php echo e(route('customer.cart.remove')); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    clearCartConfirmed() {
        if (this.isCustomer) {
            fetch('<?php echo e(route('customer.cart.clear')); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
}" x-init="init()" x-cloak><?php /**PATH C:\laragon\www\SIA_TBPrakWeb\resources\views/components/cart/cartLogic.blade.php ENDPATH**/ ?>